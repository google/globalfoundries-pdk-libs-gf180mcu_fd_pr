...
def via_stack (
    ...
    if metal_level >= 1 :
        con_gen = via_generator(x_range=x_range,y_range=y_range,via_size=con_size,via_enclosure=(con_enc,con_enc),via_layer=contact_layer,via_spacing=con_spacing)
    ...
    if metal_level >=2 :
        via1_gen = via_generator(x_range= (m1.xmin, m1.xmax), y_range=(m1.ymin,m1.ymax),via_size=via_size,via_enclosure=via_enc
        ,via_layer=via1_layer,via_spacing=via_spacing)
        via1 = c.add_ref(via1_gen)

        
        if (via1.xmax-via1.xmin + 2*m_enc[0]) < (via_size[0] + 2*via_enc[0]) and metal_level >=3:
        ...
        if (via1.ymax-via1.ymin + 2*m_enc[1]) < (via_size[1] + 2*via_enc[1]) and metal_level >=3:
