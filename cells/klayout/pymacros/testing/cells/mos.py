...
class nfet(pya.PCellDeclarationHelper):
    ...
    def produce_impl(self):
        instance = draw_nfet(self.layout, self.l, self.w, self.ld, self.nf, self.grw, self.bulk, self.volt, self.deepnwell, self.pcmpgr)
...
class pfet(pya.PCellDeclarationHelper):
    ...
    def produce_impl(self):
        instance = draw_pfet(self.layout, self.l, self.w, self.ld, self.nf, self.grw, self.bulk, self.volt, self.deepnwell, self.pcmpgr)
        write_cells = pya.CellInstArray(instance.cell_index(), pya.Trans(pya.Point(0, 0)),
                      pya.Vector(0, 0), pya.Vector(0, 0), 1, 1)
        self.cell.insert(write_cells)
        self.cell.flatten(1)
