...
class cap_nmos(pya.PCellDeclarationHelper):
    ...
    def produce_impl(self):
        np_instance = draw_cap_nmos(self.layout, self.l, self.w , self.volt, self.deepnwell, self.pcmpgr)
...
class cap_pmos(pya.PCellDeclarationHelper):
    ...
    def produce_impl(self):
        np_instance = draw_cap_pmos(self.layout, self.l, self.w , self.volt, self.deepnwell, self.pcmpgr)
        write_cells = pya.CellInstArray(np_instance.cell_index(), pya.Trans(pya.Point(0, 0)),
                      pya.Vector(0, 0), pya.Vector(0, 0), 1, 1)
