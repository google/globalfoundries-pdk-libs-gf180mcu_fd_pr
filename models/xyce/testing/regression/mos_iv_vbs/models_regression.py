...
def ext_simulated(device, vgs, vbs, vbs_sweep, sim_val):
    ...
    for i in range(0, loops):
        ...
        with open(netlist_tmp) as f:
            ...
            new_array = np.empty(
                (vbs_sweep, 1 + int(df_simulated.shape[0] / vbs_sweep))
            )
...
def error_cal(device, vgs, vbs, sim_val):
    ...
    for i in range(0, loops):
        ...
        # Mean error
        mean_error = (
            df_error[f"vbs ={vbs[0]}"].mean()
            + df_error[f"vbs ={vbs[1]}"].mean()
            + df_error[f"vbs ={vbs[2]}"].mean()
            + df_error[f"vbs ={vbs[3]}"].mean()
            + df_error[f"vbs ={vbs[4]}"].mean()
        ) / 6
        # Max error
        max_error = (
            df_error[
        ...
        df_final_ = {
            ...
            "Max error%": f'{"{:.2f}".format(max_error)} @ {max_location_vgs} & vbs (V) = {max_location_vbs}',
        }
...
def main():
    ...
    ext_simulated(
        "nfet_06v0_nvt_iv", nmos_vgs, nmos6p0_nat_vbs, mos_6p0_nat_vbs_sweep, Id_sim
    )
