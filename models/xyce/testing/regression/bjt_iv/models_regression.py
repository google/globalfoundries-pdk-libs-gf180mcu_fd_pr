...
def ext_simulated(device, vc, step, sweep, Id_sim, list_devices, ib):
    ...
    for i in range(loops):
        ...
        with open(netlist_tmp) as f:
            ...
            with open(
                f"{device}/{device}_netlists_{Id_sim}/{i}_{device}_netlist_{list_devices[k]}.spice",
                "w",
            ) as netlist:
...
def error_cal(device, vc, step, Id_sim, list_devices, ib):
    ...
    for i in range(loops):
        ...
        error_4 = round(
            100
            * abs(
                (abs(measured.iloc[0:, 4]) - abs(simulated.iloc[0:, 4]))
                / abs(measured.iloc[:, 4])
            ),
            6,
        )
        error_5 = round(
            100
            * abs(
                (abs(measured.iloc[0:, 5]) - abs(simulated.iloc[0:, 5]))
                / abs(measured.iloc[:, 5])
            ),
            6,
        )
        ...
        # Mean error
        mean_error = (
            df_error[f"{ib}{step[0]}"].mean()
            + df_error[f"{ib}{step[1]}"].mean()
            + df_error[f"{ib}{step[2]}"].mean()
            + df_error[f"{ib}{step[3]}"].mean()
            + df_error[f"{ib}{step[4]}"].mean()
        ) / 6
        # Max error
        max_error = (
            df_error[
        ...
        df_final_ = {
            ...
            "Max error%": f'{"{:.2f}".format(max_error)} @ {max_location_ib} & Vc (V) = {max_location_vc}',
        }
