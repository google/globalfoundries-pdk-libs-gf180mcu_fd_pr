...
def run_sims(
    ...
    netlists_path = os.path.join(dirpath, f"{device}_netlists_{id_rds}")
    sp_files = glob.glob(f"{netlists_path}/*.csv")
    ...
    for i in range(len(sp_files)):
        ...
        sdf.to_csv(sp_files[i], index=True, header=True, sep=",")
...
def error_cal(
    ...
    """
    calc error between measured, simulated data

    Args:
        sim_df(pd.DataFrame): Dataframe contains list of simulated data results
        meas_df(pd.DataFrame): Dataframe contains measured data
        dev_path(str):Path of output results
        id_rds(str): Simulation type
    ...
    # Rearrange meas and sim data in proper format and merging them
    for i in range(len(sim_df)):
        length = sim_df["length"].iloc[int(i)]
        width = sim_df["width"].iloc[int(i)]
        temp = sim_df["temp"].iloc[int(i)]

        sim_file_name = f"T{temp}_simulated_W{width}_L{length}.csv"
        sim_path = os.path.join(dev_path, f"{device}_netlists_{id_rds}", sim_file_name)

        simulated_data = pd.read_csv(sim_path)
        simulated_data["width"] = width
        simulated_data["length"] = length
        simulated_data["temp"] = temp
        ...
        measured_data.rename(
            columns={
                f"measured_vgs{2*i+id_rd} ={mos_vgs[0]}": mos_vgs[0],
                f"measured_vgs{2*i+id_rd} ={mos_vgs[1]}": mos_vgs[1],
                f"measured_vgs{2*i+id_rd} ={mos_vgs[2]}": mos_vgs[2],
                f"measured_vgs{2*i+id_rd} ={mos_vgs[3]}": mos_vgs[3],
                f"measured_vgs{2*i+id_rd} ={mos_vgs[4]}": mos_vgs[4],
                f"measured_vgs{2*i+id_rd} ={mos_vgs[5]}": mos_vgs[5],
            },
            inplace=True,
        )
        ...
        result_data = simulated_data.merge(measured_data, on=["Vds", "Vgs"], how="left")

        result_data["relative_err"] = (
            np.abs(result_data[f'{id_rds}_meas'] - result_data[f'{id_rds}_sim'])
            * 100.0
            / (result_data[f'{id_rds}_meas'])
        )
        result_data["relative_err"].fillna(0, inplace=True)

        if id_rds == "Id":
            # clipping all the  values to lowest_curr
            result_data[f'{id_rds}_meas'] = result_data[f'{id_rds}_meas'].clip(lower=CLIP_CURR)
            result_data[f'{id_rds}_sim'] = result_data[f'{id_rds}_sim'].clip(lower=CLIP_CURR)
