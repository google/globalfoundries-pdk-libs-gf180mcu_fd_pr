    ...
    for i in range(start, 4 + start):
        ...
        with open(netlist_tmp) as f:
            ...
            with open(
                ...
                netlist.write(
                    tmpl.render(
                        device=device, width=width, length=length, corner=corner
                    )
                )
            ...
            df_simulated = pd.read_csv(
                f"{dirpath}/{device}_netlists_{cv_sim}/{i-start}_{device}_netlist_w{width}_l{length}.spice.log"
            )
            clean_data = (
                str(df_simulated.loc[4])
                .replace("Compatibility modes selected: hs", "")
                .replace("\nName: 4, dtype: object", "")
                .split("=")
            )
...
def main():
    ...
    for corner in corners:
        for device in devices:
            ...
            read_file = pd.read_excel(
                f"../../180MCU_SPICE_DATA/Cap/moscap_cv_3p3.nl_out.xlsx"
            )
    ...
    for corner in corners:
        for device in devices:
            ...
            # From xlsx to csv
            read_file = pd.read_excel(
                f"../../180MCU_SPICE_DATA/Cap/moscap_cv_6p0.nl_out.xlsx"
            )
            read_file.to_csv(f"{dirpath}/{device}.csv", index=False, header=True)
