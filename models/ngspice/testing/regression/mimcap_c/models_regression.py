...
def ext_measured(dev_data_path, device):
    ...
    idf["length"] = df["Unnamed: 2"].apply(lambda x: str(x).split("(")[1].split("x")[0].split("u")[0])
    idf["width"] = df["Unnamed: 2"].apply(lambda x: str(x).split("(")[1].split("x")[1].split("u")[0])
    ...
    idf["device"] = idf["device"].apply(lambda x: str(x).replace("mim_1p5fF", "cap_mim_1f5"))
    idf["device"] = idf["device"].apply(lambda x: str(x).replace("mim_1p0fF", "cap_mim_1f0"))
    idf["device"] = idf["device"].apply(lambda x: str(x).replace("mim_2p0fF", "cap_mim_2f0"))
    ...
    for i in range(0,loops):
        ...
        if i in range (0,corner_count):
            ...
            if i in range (2*corner_count,2*corner_count+temp_count):
                temp = 25
            elif i in range (2*corner_count+temp_count, 2*corner_count+2*temp_count):
                temp = -40
            else:
                temp = 175             
        else:
...
def run_sim(dirpath, device, length, width, corner, temp):
    ...
    netlist_path = f"{dirpath}/{device}_netlists/netlist_w{width}_l{length}_t{temp}_{corner}.spice"
...
def main():
    ...
    for i, dev in enumerate(devices):
        ...
        logging.info(f"# Device MIM {dev} number of measured_datapoints : {len(meas_df)}")

        sim_df = run_sims(meas_df, dev_path, workers_count)
        logging.info(f"# Device MIM {dev} number of simulated datapoints :{len(sim_df)}")
