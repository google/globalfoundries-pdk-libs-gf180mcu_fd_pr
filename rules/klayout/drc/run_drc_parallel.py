...
def combine_results(path, rule_decks):
    ...
    for i, rule_deck in enumerate(rule_decks):
        if i == 0:
            continue
        with open(
            f"{name_clean_}_main_drc_gf{arguments['--gf180mcu']}_{i}.lyrdb", "r"
        ) as f:
            for line in f:
                if "<cells>" in line:
                    get_category = False
    ...
    with open(
        f"{name_clean_}_main_drc_gf{arguments['--gf180mcu']}_{0}.lyrdb", "r"
    ) as f:
...
def get_top_cell_names(gds_path):
    ...
    proc = subprocess.Popen(
        [
            ...
            "-b",
            "-r",
            f"{pdk_root}/{pdk}/utils/get_top_cell_names.rb",
            "-rd",
...
def clean_gds_from_many_top_cells(gds_path, topcell):
    ...
    proc = subprocess.Popen(
        [
            ...
            "-b",
            "-r",
            f"{pdk_root}/{pdk}/utils/keep_single_top_cell.rb",
            "-rd",
            "infile={}".format(gds_path),
            "-rd",
...
def main():
    ...
    if arguments["--path"]:
        ...
        if ".gds" in path:
            ...
            if (arguments["--antenna_only"]) and not (arguments["--density_only"]):
                ...
                os.system(
                    f"klayout -b -r $PDK_ROOT/$PDK/gf180mcu_antenna.drc -rd input={path} -rd report={name_clean}_antenna_gf{arguments['--gf180mcu']}_gf{arguments['--gf180mcu']}.lyrdb -rd thr={thrCount} {switches}"
                )
                ...
                os.system(
                    f"klayout -b -r $PDK_ROOT/$PDK/gf180mcu_density.drc -rd input={path} -rd report={name_clean}_density_gf{arguments['--gf180mcu']}.lyrdb -rd thr={thrCount} {switches}"
                )
                ...
                os.system(
                    f"klayout -b -r $PDK_ROOT/$PDK/gf180mcu_antenna.drc -rd input={path} -rd report={name_clean}_antenna_gf{arguments['--gf180mcu']}.lyrdb -rd thr={thrCount} {switches}"
                )
                ...
                rule_decks = os.listdir(
                    f"{os.environ['PDK_ROOT']}/{os.environ['PDK']}/rule_decks/"
                )
                for i, rule_deck in enumerate(rule_decks):
    ...
    # ======================== Reporting results ========================
    rule_deck_path = [
        f"{pdk_root}/{pdk}/gf180mcu.drc",
        f"{pdk_root}/{pdk}/gf180mcu_antenna.drc",
        f"{pdk_root}/{pdk}/gf180mcu_density.drc",
    ]
